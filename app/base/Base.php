...
class Base
{
     ...
     * 这个值一般都是定义为常量的
     */
    public function getErrorOut($code)
    {
        $arrConf = \wyCupboard::$config;
        $codeMap = $arrConf['errorCode'];
     ...
     * @param $outData
     */
    public function jsonOut($outData)
    {
        $isLogger = self::isLogger();
        $isLogger && \wyCupboard::$logger->addNode('response', $outData);
        \app\helper\OutPut::jsonOut($outData);
        die;
    }
    
    /**
     * 判断是否记录项目日志的response节点
     *
     * @return bool
     */
    protected function isLogger()
    {
        
        $unLogger = true;
        $requestMethod = !empty($_SERVER['REQUEST_METHOD']) ? $_SERVER['REQUEST_METHOD'] : '';
        $func = @(\wyCupboard::$request->controller . '_' . \wyCupboard::$request->action);
        if (empty($requestMethod)) {
            return $unLogger;
        }
        $excludeServices = !empty(\wyCupboard::$config['logExclude']['app']) ? \wyCupboard::$config['logExclude']['app'] : [];
        //GET走白名单
        switch ($requestMethod) {
            case  'GET':
                if ( !empty($excludeServices[$requestMethod]) && in_array($func, $excludeServices[$requestMethod])) {
                    $unLogger = true;
                } else {
                    $unLogger = false;
                }
     ...
     * @param $data
     */
    function jsonpOut($data, $callback = 'wepiao_mqq')
    {
     ...
     * @param int 错误码
     */
    function jsonError($code = -1, $msg = '')
    {
        $data = ['ret' => $code, 'sub' => $code, 'msg' => $msg];
        self::jsonOut($data);
    }
     ...
     */
    public function getParam(array $input, $key = '', $default = '')
    {
        //如果要取appkey的值，先取$input['appkey']，若无则取$input['channelId']，再无则取$default
        if ($key == 'appkey') {
            $channelId = empty( $input['channelId'] ) ? $default : $input['channelId'];
            $appkey = empty( $input['appkey'] ) ? $channelId : $input['appkey'];
    ...
    public function http($sUrl, $params = [], $isPostRequestId = true)
    {
        ...
        $arrHeader = [
            ...
            'charset=utf-8',
            'User-Ip: ' . \app\helper\Net::getRemoteIp(), //用户ip，后端记录用
        ];
        //增加需要传到后端的Header信息(如果值为空则不传)
        $arrParts = [
            'X-User-IDFA',
            'X-User-IMEI',
            'X-User-MAC',
        ...
        \wyCupboard::$logger->addRequestOtherNode([
            'url'             => $sUrl,
            'method'          => $sMethod,
            'data'            => $strData,
            'cookie'          => $strCookies,
        ...
        if ($response['http_code'] == '200') {
            ...
            $arrReturn = ['ret' => '50401', 'sub' => '50401', 'msg' => $response['http_errmsg']];
    ...
    {
        $requestId = \app\helper\Utils::getRequestId();
        $requestId = !empty( $requestId ) ? $requestId : mt_rand(1, 999999) . "_" . time();
        if (isset( \wyCupboard::$request->params['X-Request-Id'] )) {
            $requestId = \wyCupboard::$request->params['X-Request-Id'];
        }
     ...
     * @return array
     */
    public function getCacheData($params = '', $expire = 300)
    {
        $switch = \wyCupboard::$config['cacheData'];
        if (!empty($switch)) {
    ...
    private function getCacheDataFilename($params = '')
    {
        ...
        $cacheFilename = \wyCupboard::$request->controller . '_' . \wyCupboard::$request->action;
        if (!empty($params)) {
